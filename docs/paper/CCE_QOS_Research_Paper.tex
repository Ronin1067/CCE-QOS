...
To support modern Large Language Model (LLM) inference on edge NPUs, CCE-QOS extends the CCE QUBO formulation to dynamic Key-Value (KV) cache block allocation. By co-scheduling shared prompt prefixes and overlapping decode token updates under on-chip SRAM bounds, CCE QUBO eliminates 56.0\% of off-chip DRAM page faults, achieving a 55.21\% energy reduction and a 3.54$\times$ reduction in inter-token latency (ITL) over standard LRU PagedAttention.
